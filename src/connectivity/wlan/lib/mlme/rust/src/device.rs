...
mod tests {
    ...
    fn start_hw_scan() {
        ...
        let result = dev.start_hw_scan(&WlanHwScanConfig {
            ...
            ssid: WlanSsid { len: 3, ssid: arr!([65; 3], fidl_ieee80211::MAX_SSID_LEN as usize) },
        });
        assert!(result.is_ok());
        assert_variant!(fake_device.hw_scan_req, Some(config) => {
            ...
            assert_eq!(config.ssid, WlanSsid { len: 3, ssid: arr!([65; 3], fidl_ieee80211::MAX_SSID_LEN as usize) });
